...
Public Class frmItemCategories
    ...
    End Sub
    Sub FillGrid()
        Grid.DataSource = D.GetDataTable("Select * from tblItemCategories Order By CatID")
    ...
    Function GetNewID() As String
        ...
        Dim S As String
        Dim N As Integer
        S = D.GetScaler("Select Max(CatID) from tblItemCategories")
        If S = "" Then
    ...
    End Function
    Sub SaveRec()
        Dim QB As New QueryBuilder("tblItemCategories")
        Dim strSQL As String
        With QB
            .AddField("catID", txtCode.Text, FieldType.Text)
            .AddField("catName", txtName.Text, FieldType.Text)
        End With
        If DM = DAL.DAL.DataMode.NewRecord Then
            strSQL = QB.GetInsertQuery
        ElseIf DM = DAL.DAL.DataMode.EditRecord Then
            QB.RemoveField("CatID")
            QB.InsertDeleteFilterField("CatID", txtCode.Text, FieldType.Text)
        ...
        Me.Cursor = Cursors.Default
        FillGrid()
        MsgBox("Data of is saved", MsgBoxStyle.Information, CName)
        ClearAll()
        Lockall(True)
        cmdAdd.Focus()
        DM = DAL.DAL.DataMode.SavedRecord
    ...
    Private Sub Grid_SelectionChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Grid.SelectionChanged
        With Grid
            If .Rows.Count > 0 Then
                FillData(.Rows(.CurrentRow.Index).Cells("colCatID").Value)
            End If
        End With
    End Sub
    Sub FillData(ByVal ID As String)
        ...
        DT = D.GetDataTable("Select * from tblItemCategories Where catID = " & ID)
        With DT
            If DT.Rows.Count > 0 Then
                txtCode.Text = .Rows(0).Item("CatID")
                txtName.Text = .Rows(0).Item("CatName")
            End If
        End With
        Me.Cursor = Cursors.Default
    End Sub
